...
#View(data)
# Create a subset of the dataset where 'injured' is equal to 8
#subset_df <- data[data$NUMBER.OF.PERSONS.INJURED == 4, ]
# View the subset of the dataset
#View(subset_df)
#colnames(data)
...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      /* Style the tabs and selected tab */
       #raw_data {
      ...
      .nav.nav-tabs > li.active > a {
        background-color: lightblue !important; /* Active tab background color */
        color: black !important; /* Active tab text color */
      }
      .nav.nav-tabs > li > a {
        background-color: lightgreen !important; /* Inactive tab background color */
        color: black !important; /* Inactive tab text color */
      }
      .sidebar {
  ...
  titlePanel(HTML("<h1 style='text-align: center; color: darkred; font-weight: bold;'>Vehicle Collision Analysis</h1>")),
  sidebarLayout(
    sidebarPanel(
      div(style = "font-size: 12px; color: darkblue; font-style: italic;",
        "Upload your own data and set filters for visualizing vehicle collisions.",
      fileInput("file", "Upload Your Own Data")),
      div(style = "font-size: 12px; color: darkblue; font-style: italic;",
      sliderInput("injured_people", "Number of persons injured in vehicle collisions", 0, 7, 0)),
      div(style = "font-size: 12px; color: darkblue; font-style: italic;",
      sliderInput("hour", "Select Hour", min = 0, max = 23, value = 0)),
      div(style = "font-size: 12px; color: darkblue; font-style: italic;",
        "Select the type of victims you want to analyze for top fatal streets.",
      selectInput("affected_class", "Death Victim Type (Top Fatal Streets)", 
                  choices = c("Dead Motorists", "Dead Pedestrians", "Dead Cyclists"))
      ),
      div(style = "font-size: 12px; color: darkblue; font-style: italic;",
        "Select the type of victims you want to analyze for riskiest streets.",
      selectInput("affected_injured_class", "Injured Victim Type (Riskiest Streets)", 
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    ...
    injured_people_ <- input$injured_people
    map_title <- sprintf("<span style='color: darkred;'><em><strong>Streets with %d Injured Persons</strong></em></span>", injured_people_)
    
    data_filtered_df <- data_filtered_df[data_filtered_df$`NUMBER.OF.PERSONS.INJURED` == injured_people_, ]
  ...
  output$hourly_map <- renderLeaflet({
    ...
    leaflet(data_filtered_df) %>%
      ...
      addLegend(
        position = "bottomright",
        colors = "darkorange",
        labels = "Hourly Street Collisions"
      )
  })
  ...
  output$streets_by_deaths <- renderDataTable({
    ...
    ), caption = "*Top Five Cities by Total number of Deaths*")
  })
